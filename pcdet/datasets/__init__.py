...
def build_dataloader(dataset_cfg, 
    ...
    if validation:
        validation_set = __all__[dataset_cfg.DATASET](
        ...
        if dist:
            if training:
                sampler_validation = torch.utils.data.distributed.DistributedSampler(validation_set)
            else:
        ...
        dataloader_validation = DataLoader(
            ...
            pin_memory=True,
            num_workers=workers,
            shuffle=(sampler_validation is None) and validation,
            collate_fn=validation_set.collate_batch,
            drop_last=False,
            sampler=sampler_validation,
            timeout=0
        ) 
        return validation_set, dataloader_validation, sampler_validation
...
def build_active_dataloader(dataset_cfg, 
                            class_names, 
                            batch_size, 
                            dist, 
                            root_path=None, 
    ...
    unlabelled_set = __all__[dataset_cfg.DATASET](
        ...
        class_names=class_names,
        root_path=root_path,
        training=training,     # set to True, if you're using the train or val sets
        validation=False,
        logger=logger,
    )
    ...
    if active_training is not None:
        ...
        if cfg.DATA_CONFIG.DATASET == 'WaymoDataset':
            labelled_set.frame_ids, labelled_set.infos = active_training[0], active_training[1]
            unlabelled_set.frame_ids, unlabelled_set.infos = active_training[2], active_training[3]
        
        elif cfg.DATA_CONFIG.DATASET == 'TUMTrafDataset':
            labelled_set.sample_id_list, labelled_set.tumtraf_infos = active_training[0], active_training[1]
            unlabelled_set.sample_id_list, unlabelled_set.tumtraf_infos = active_training[2], active_training[3]

        else: # kitti cases
            labelled_set.sample_id_list, labelled_set.kitti_infos = active_training[0], active_training[1]
            unlabelled_set.sample_id_list, unlabelled_set.kitti_infos = active_training[2], active_training[3]
        ...
        if cfg.DATA_CONFIG.DATASET == 'WaymoDataset':
            ...
            labelled_pairs = list(zip(dataset.sample_id_list, dataset.tumtraf_infos))
            random.shuffle(labelled_pairs)

            unlabelled_pairs = list(zip(unlabelled_set.sample_id_list, unlabelled_set.tumtraf_infos))
            random.shuffle(unlabelled_pairs)

            labelled_set.sample_id_list, labelled_set.tumtraf_infos = zip(*labelled_pairs[:cfg.ACTIVE_TRAIN.PRE_TRAIN_SAMPLE_NUMS])
            unlabelled_set.sample_id_list, unlabelled_set.tumtraf_infos = zip(*labelled_pairs[cfg.ACTIVE_TRAIN.PRE_TRAIN_SAMPLE_NUMS:])
            ...
            labelled_set.sample_id_list, labelled_set.kitti_infos = zip(*pairs[:cfg.ACTIVE_TRAIN.PRE_TRAIN_SAMPLE_NUMS])

            unlabelled_set.sample_id_list, unlabelled_set.kitti_infos = zip(*pairs[cfg.ACTIVE_TRAIN.PRE_TRAIN_SAMPLE_NUMS:])
    ...
    dataloader_labelled = DataLoader(
        ...
        pin_memory=True, 
        num_workers=workers,
        shuffle=(sampler_labelled is None) and training, 
    ...
    dataloader_unlabelled = DataLoader(
        ...
        pin_memory=True, 
        num_workers=workers,
        shuffle=(sampler_unlabelled is None) and training, 
