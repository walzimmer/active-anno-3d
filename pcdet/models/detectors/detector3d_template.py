...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing_proannoV2(self, batch_dict):
        DENSE_HEAD = self.model_cfg.DENSE_HEAD.NAME

        post_process_cfg = self.model_cfg.POST_PROCESSING
        batch_size = batch_dict['batch_size']
        recall_dict = {}
        pred_dicts = []
        if DENSE_HEAD == 'AnchorHeadSingle':
            bbox_classes = [i['class_name'] for i in self.model_cfg.DENSE_HEAD.ANCHOR_GENERATOR_CONFIG]
        else:
            bbox_classes = [i for i in self.model_cfg.DENSE_HEAD.CLASS_NAMES_EACH_HEAD[0]]

        if 'rcnn_cls' in batch_dict.keys() and len(batch_dict['rcnn_cls'].shape) > 2:
            rcnn_cls = torch.mean(torch.sigmoid(batch_dict['rcnn_cls']), 0).view(batch_size, -1, 1)
            rcnn_reg = torch.mean(batch_dict['rcnn_reg'], 0).view(batch_size, -1, 7)
        else:
            rcnn_cls = None
            rcnn_reg = None
        ...
        for index in range(batch_size):
            ...
            if batch_dict['batch_box_preds'].shape.__len__() == 4:
            ...
            if DENSE_HEAD == 'AnchorHeadSingle':
                ...
                full_cls_scores = batch_dict['pred_logits'][batch_mask]
            ...
            if self.model_cfg.get('ROI_HEAD.EMBEDDING_REQUIRED', None):
                shared_features = batch_dict['shared_features']
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                selected_scores = cls_confs
                if post_process_cfg.OUTPUT_RAW_SCORE:
                    max_cls_preds, _ = torch.max(src_cls_preds, dim=-1)
                    selected_scores = max_cls_preds

            pred_boxes_reshaped = box_preds.reshape(1, -1, 7)
            box_volumes = box_preds[:, 3] * box_preds[:, 4] * box_preds[:, 5]
            pred_box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(sampled_points,
                                                                             pred_boxes_reshaped).long().squeeze(dim=0)
            pts_counts = torch.zeros(box_preds.shape[0] + 1).cuda()
            mask = pred_box_idxs_of_pts != -1
            pred_box_idxs_of_pts = pred_box_idxs_of_pts[mask]
            unique_box_id, pred_box_unique_pts_counts = torch.unique(pred_box_idxs_of_pts, return_counts=True)
            ...
            pred_box_unique_density[pred_box_unique_density == 0] = 1.0

            record_dict = {
                'confidence': cls_preds,
                'rpn_preds': batch_dict['rpn_preds'] if DENSE_HEAD == 'AnchorHeadSingle' else batch_dict['hm_preds'],
                'batch_rcnn_cls': batch_rcnn_cls,
                'batch_rcnn_reg': batch_rcnn_reg,
                'embeddings': shared_features,
                'pred_logits': full_cls_scores,
    ...
    def post_processing_AL(self, batch_dict):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        for index in range(batch_size):
            ...
            # shared_features = batch_dict['shared_features'] if 'sharefinal_boxesd_features' in batch_dict.keys() else None
            if self.model_cfg.get('ROI_HEAD.EMBEDDING_REQUIRED', None):
                shared_features = batch_dict['shared_features']
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                if not isinstance(cls_preds, list):
                    selected, selected_scores = class_agnostic_nms(
                        box_scores=cls_confs, 
                        box_preds=box_preds,
                        nms_config=post_process_cfg.NMS_CONFIG,
                        score_thresh=post_process_cfg.SCORE_THRESH
                    )
                else:
                    output = [class_agnostic_nms(
    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        """
        Calculates and records the number of true positives (recalls) for different Intersection over Union (IoU) 
        thresholds for both the Region of Interest (RoI) and the Region-based Convolutional Neural Networks (RCNN).

        Parameters:
        ----------
            box_preds (torch.Tensor): The predicted bounding boxes from the PV-RCNN model.
            recall_dict (dict): A dictionary that records the total number of ground truth boxes and the number of 
                                recalled boxes for different IoU thresholds.
            batch_index (int): The index of the current batch being processed.
            data_dict (dict, optional): A dictionary that includes the ground truth boxes and the RoI boxes.
            thresh_list (list, optional): A list of IoU thresholds for calculating recalls.

        Returns:
        -------
            dict: The updated recall_dict with the current batch's recalls added.
        """
