...
class PVRCNN(Detector3DTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            op = self.model_cfg.OPERATION
            if (op == 'train_active_all') or (op == 'train_active_latest') or (op == 'query_only'):
                pred_dicts, recall_dicts = self.post_processing_proannoV2(batch_dict)
            
            elif (op == 'inference'):
                pred_dicts, recall_dicts = self.post_processing(batch_dict)
                
            else:
                pred_dicts, recall_dicts = self.post_processing_AL(batch_dict)
