...
from tools.utils.eval_utils import eval_utils
...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default='/ahmed/output/active-tumtraf_models/pv_rcnn_active_crb/tunedAll/pv_rcnn_active_crb.yaml', 
    ...
    parser.add_argument('--eval_backbone', action='store_true', default=False, help='whether to evaluate the intial checkpoint')
    ...
    cfg.EXP_GROUP_PATH = '/'.join(args.cfg_file.split('/')[0:-1])  # remove 'cfgs' and 'xxxx.yaml'
...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, dist_test=False):
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
    ...
    if cfg.LOCAL_RANK == 0:
        ...
        if cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] == 'waymo_dataset':
            wandb.init(project=cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] + '_test', entity='ahmedalaa-gh1')

        elif cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] == 'kitti_dataset':
            wandb.init(project=cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] + '_test', entity='ahmedalaa-gh1')

        elif cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] == 'tumtraf_dataset':
            wandb.init(project=cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] + '_test', entity='ahmedalaa-gh1')
...
def repeat_eval_ckpt_new(model,
                         test_loader,
                         args,
                         eval_output_dir,
                         logger,
    ...
    e_pre = cfg.ACTIVE_TRAIN.PRE_TRAIN_EPOCH_NUMS
    e_int = cfg.ACTIVE_TRAIN.SELECT_LABEL_EPOCH_INTERVAL
    select_nums = cfg.ACTIVE_TRAIN.SELECT_NUMS
    pretrain_sample_n = cfg.ACTIVE_TRAIN.PRE_TRAIN_SAMPLE_NUMS
    
    run_name_elements = [dataset_name] + [cfg.TAG] + [e_pre] + [e_int] + [pretrain_sample_n] + [select_nums]
    run_name_elements = '_'.join([str(i) for i in run_name_elements])
    wandb.run.name = run_name_elements
    wandb.config.update(args)
    wandb.config.update(cfg)
    ...
    for cur_ckpt in ckpt_list:
        cur_epoch_id = cur_ckpt.split('.')[0].split('_')[-1]
...
def main():
    ...
    if args.eval_backbone:
        ...
        # output_dir = cfg.ROOT_DIR / 'output' / cfg.EXP_GROUP_PATH 
        output_dir = Path(cfg.EXP_GROUP_PATH)
        # output_dir.mkdir(parents=True, exist_ok=True)
    ...
    with torch.no_grad():
        if args.eval_all:
            repeat_eval_ckpt_new(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=dist_test)

        else:
            eval_single_ckpt(model, test_loader, args, eval_output_dir, 
                             logger, dist_test=dist_test)
