...
def detections_to_openlabel(
    ...
    with open(os.path.join(output_folder_path, filename), "w", encoding="utf-8") as f:
        json.dump(output_json_data, f, indent=4, default=handle_float32)

    return output_json_data


def process_detections(det_file_path, save_path):
    import pickle
    with open(det_file_path, 'rb') as file:
        data = pickle.load(file)
    
    
    for dp in data:
        dets = []
        frame_id = dp['frame_id']
        for i in range(len(dp['name'])):
            box = dp['boxes_3d'][i] 
            det_obj = Detection(
                location=np.array([box[0], box[1], box[2]]),
