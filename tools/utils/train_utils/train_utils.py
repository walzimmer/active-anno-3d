...
from tools.utils.eval_utils.eval_utils import load_data_to_gpu, add_avg_performance, statistics_info


def val_one_epoch(cfg,
                  model,
                  dataloader,
                  epoch_id, 
                  logger):
    ...
    logger.info('*************** EPOCH %s VALIDATION *****************' % epoch_id)
...
def train_model(model, 
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            # if optim_cfg.VALIDATION and trained_epoch % optim_cfg.VALID_EPOCH_INTERVAL == 0:

            #     tb_dict = val_one_epoch(
            #         cfg=cfg,
            #         model=model,
            ...
            #         logger=logger
            #     )
            #     tb_dict = {key.replace('/', '_'): val for key, val in tb_dict.items()}
            #     tb_dict = {key: val for key, val in tb_dict.items() if key.split('_')[-1] == 'overall' or \
            #                 key.split('_')[0] == 'recall'}
            #     for key, val in tb_dict.items():
...
def resume_dataset(labelled_loader, 
                   unlabelled_loader, 
    ...
    if cfg.DATA_CONFIG.DATASET == 'KittiDataset':
        ...
        pairs = list(zip(unlabelled_set.sample_id_list, unlabelled_set.tumtraf_infos))
    ...
    if cfg.DATA_CONFIG.DATASET == 'KittiDataset':
        ...
        selected_id_list, selected_infos = list(labelled_set.sample_id_list), \
                                            list(labelled_set.tumtraf_infos)
        unselected_id_list, unselected_infos = list(unlabelled_set.sample_id_list), \
                                                list(unlabelled_set.tumtraf_infos)
    else: # Waymo
    ...
    # @TODO: 
    if cfg.ACTIVE_TRAIN.NETWORK_UPDATE.DATASET == 'ALL_AVAILABLE_DATA':
    ...
    else:   # CURRENT_SELECTED_DATA
        file_dir = selected_frames_list[-1]
        pkl_file = pickle.load(open(file_dir, 'rb'))
        frame_list = [str(i) for i in pkl_file['frame_id']]
        selected_frames_files += frame_list
        print('successfully load the selected frames from {}'.format(file_dir.split('/')[-1]))

    # TODO: Integrate the case when training last round on full acquired data
    # for file_dir in selected_frames_list:
    #     pkl_file = pickle.load(open(file_dir, 'rb'))
    #     frame_list = [str(i) for i in pkl_file['frame_id']]
    #     selected_frames_files += frame_list
    ...
    return labelled_loader, unlabelled_loader, selected_frames_files
