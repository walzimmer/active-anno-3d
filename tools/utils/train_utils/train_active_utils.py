...
from pcdet.utils.active_training_utils import select_active_labels, prepare_network_and_dataset
from pcdet.config import cfg
from pcdet.models import build_network

from tools.utils.eval_utils.eval_utils import eval_one_epoch
from tools.utils.train_utils.optimization import build_scheduler, build_optimizer
from tools.utils.train_utils.train_utils import model_state_to_cpu, save_checkpoint, checkpoint_state, resume_dataset
from tools.utils.eval_utils.eval_utils import load_data_to_gpu
...
def val_one_epoch(model, 
    ...
    if rank == 0:
            pbar = tqdm.tqdm(total=total_iter, desc='val', dynamic_ncols=True)

    model.eval()

    
    for cur_it in range(total_iter):
        ...
        total_loss['loss'] += loss_item
        disp_dict.update({'loss': loss_item})


        if rank == 0:
            pbar.update()
            pbar.set_postfix(dict(total_it=history_accumulated_iter))
    ...
    average_loss = {key: val / total_iter for key, val in total_loss.items()}
    for key, val in average_loss.items():
        tb_log.add_scalar('val/' + key, val, history_accumulated_iter)
        wandb.log({'val/' + key: val}, step=history_accumulated_iter)
...
def train_model_active(model, 
    ...
    if cfg.ACTIVE_TRAIN.TRAIN_RESUME:
        ...
        if len(selected_frames_list) > 0:
            labelled_loader, unlabelled_loader, _ = resume_dataset(labelled_loader, 
                                                               unlabelled_loader, 
                                                               selected_frames_list, 
                                                               dist_train, logger, 
                                                               cfg)
        ...
        with tqdm.trange(start_epoch, active_pre_train_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
            ...
            for cur_epoch in tbar:
                ...
                if  trained_epoch % ckpt_save_interval == 0 and rank == 0:
                    ...
                    save_checkpoint(
                        checkpoint_state(model, optimizer, trained_epoch, accumulated_iter, lr_scheduler), 
                        filename=ckpt_name,
                    )
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True,
        ...
        for cur_epoch in tbar:
            ...
            if cur_epoch == active_pre_train_epochs or ((cur_epoch - active_pre_train_epochs) % cfg.ACTIVE_TRAIN.SELECT_LABEL_EPOCH_INTERVAL == 0):
                ...
                selected_frames, strategy, labelled_loader, unlabelled_loader = select_active_labels(
                    model,
                    labelled_loader,
                    unlabelled_loader,
                    rank,
                ...
                # @TODO: integrate the case when tunedLatest_finalAll
                labelled_loader, unlabelled_loader = prepare_network_and_dataset(
                    model=model,
                    strategy=strategy,
                ...
                lr_scheduler, lr_warmup_scheduler = build_scheduler(
                    ...
                    total_epochs=cfg.ACTIVE_TRAIN.SELECT_LABEL_EPOCH_INTERVAL,
            ...
            new_accumulated_iter, accumulated_iter = train_one_epoch(model, 
                                                                    optimizer, 
                                                                    labelled_loader, 
                                                                    model_func,
                                                                    lr_scheduler=cur_scheduler,
                                                                    accumulated_iter=new_accumulated_iter, 
                                                                    optim_cfg=optim_cfg,
                                                                    rank=rank, 
                                                                    tbar=tbar, 
                                                                    tb_log=tb_log,
